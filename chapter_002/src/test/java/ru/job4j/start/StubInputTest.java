...
public class StubInputTest {
    ...
    public void whenUserAddItemThenTrackerHasNewItemWithSameName() {
        ...
        Input input = new StubInput(new String[]{"0", "test name", "desc", "123456789", "да"});
    ...
    public void whenUpdateThenTrackerHasUpdatedValue() {
        ...
        Input input = new StubInput(new String[]{"2", item.getId(), "test name", "desc", "123456", "да"});
        new StartUI(input, tracker).init();
    ...
    public void whenUserDeleteItemThenTrackerReturnAllItems() {
        ...
        Input input = new StubInput(new String[]{"3", first.getId(), "да"});
        new StartUI(input, tracker).init();
        Item[] result = {};
        assertThat(tracker.findAll(), is(result));
    }
    ...
    public void whenUserFindItemByIdThenTrackerReturnFoundItem() {
        ...
        Input input = new StubInput(new String[]{"5", item.getId(), "да"});
        new StartUI(input, tracker).init();
        assertThat(tracker.findById(item.getId()).getName(), is("test name"));
    }
    /**
    ...
    public void whenUserFindItemsByNameThenTrackerReturnItems() {
        Tracker tracker = new Tracker();
        Item item = tracker.add(new Item("test name", "test desc", 123L));
        Input input = new StubInput(new String[]{"4", item.getName(), "да"});
        new StartUI(input, tracker).init();
        Item[] result = {item};
